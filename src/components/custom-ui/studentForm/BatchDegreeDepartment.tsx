...
interface BatchDegreeDepartmentProps {
...
export const BatchDegreeDepartment: React.FC<BatchDegreeDepartmentProps> = ({
    control,
    name,
    label,
    apiUrl,
    ...
    const fetchOptions = useCallback(
            ...
            try {
                ...
                if (search.trim() !== "") {
                    const filters = encodeURIComponent(
                        JSON.stringify({
                            [`${valueFieldPath}__value`]: { $contains: search },
                        })
                    );
                    url += `&filters=${filters}`;
                }
                ...
                const formattedOptions = data.items.map((item) => ({
                    id: item.id,
                    value: (item[valueFieldPath as keyof Option] as any)?.value ?? "",
                }));

                if (pageToFetch === 1) {
                    setOptionsData(formattedOptions);
                } else {
                    setOptionsData((prev) => [...prev, ...formattedOptions]);
                }
        ...
        [apiUrl, CustomHeader, disabled, valueFieldPath] // 👈 must watch valueFieldPath too
    );
